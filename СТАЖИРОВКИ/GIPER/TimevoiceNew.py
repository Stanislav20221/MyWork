...
dialog = '''
    ...
    12.  Программа лояльности (Вопросы по программе лояльности, списаниям баллов, возвратам баллов, сертификатам, бонусам и т. п.).
    14.  Юридическое лицо (если вопрос входит в зону ответственности АСЦ и клиент является юридическим лицом – компанией, ООО, ИП, и т. п.).
...
def answer_index(system, topic, dialog, temp, verbose=0):
    ...
    try:
        messages = [  # <-- Теперь тут 4 пробела
            ...
            {"role": "user", "content": f"Проанализируй текст диалога клиента и менеджера и выбери наиболее точную категорию из списка: {topic}. Выведи только тему зонка. Если звонок не относится ни к одной теме (например, вопрос обучения, предложения сотудничества, партнерские отношения и т. д.), пометь его, как нецелевой звонок. Тебе категорически запрещено в ответе выводить пояснения. Ничего не придумывай от себя. Диалог {dialog}"},
            {"role": "assistant", "content": "Проблема с купленным товаром."}
        ]
