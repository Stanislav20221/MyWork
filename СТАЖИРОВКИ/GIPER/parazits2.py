...
def answer_index(system, topic, dialog, temp, verbose=0):
    ...
    try:
        messages = [  # <-- Теперь тут 4 пробела
            ...
            {"role": "user", "content": f"""
                                        Проанализируй текст диалога клиента и менеджера (Speaker 2 - это говорит клиент  Speaker 1 - это говорит менеджер) и выяви наличие нежелательных слов в диалоге менеждера. 
                                        Выведи информацию о наличии/отсутствии и общего количества этих слов в диалоге менеджера по каждой категории. Поставь оценку работе менеджера, основываясь на следующих правилах:
                                        100% (14 баллов) - В речи менеджера нет ошибок: на протяжении всего разговора сохраняется деловой стиль общения. Оператор не перебивает абонента, не говорит параллельно и не спорит с ним, не обвиняет клиента. Не комментирует свои действия (если клиент НЕ на удержании). Отсутствуют формулировки «Я не знаю», «я не уверен», «насколько я помню», «если я не ошибаюсь» и т.д. В речи отсутствуют просторечные слова и выражения, жаргон, слова-паразиты, и уменьшительно-ласкательные формы, обилие междометий; фразы и предложения построены правильно, слова употребляются в нужном контексте, нет ошибок в ударениях.
                                        50% (7 балла) - В речи менеджера 1-2 ошибки за разговор. Оператор использует сложные речевые конструкции или фразы "к сожалению", "вас беспокоит ..." и т.п. Речь монотонная.
                                        0% (0 баллов) - Оператор перебивает клиента, говорит в параллель, путает имя клиента. Оператор более 3 раз за разговор нарушает деловой стиль общения, совершает грамматические ошибки и/или использует некорректные фразы.
                                        На основании оценки, предложи рекомендации по улучшению качества работы менеджера.
                                        Оценку работы менеджера и рекомендации отметь словосочетаниями "Оценка работы менеджера" и "Рекомендации по улучшению качества работы менеджера".
                                        Результаты выведи в текстовом и JSON формате (для JSON формата выведи общее количество по каждой категории). 
                                        Перед выводом каждого формата обозначь 'Текстовый формат:' 'JSON формат:' 'Список нежелательных слов: {topic} Текст: {dialog} Важно! Нежелательные слова и слова-паразиты выяви только в диалоге менеджера (Speaker 1). 
                                        Тебе запрещено выявлять их в диалоге клиента. Ничего не придумывай от себя"
                                        """}
        ]
...
#=================================================================================== ФУНКЦИЯ ВЫВОДА РЕЗУЛЬТАТОВ РАБОТЫ МЕНЕДЖЕРА (БЕЗ ОЦЕНКИ И РЕКОМЕНДАЦИЙ) ==============================================================================================================

def text_result(text, phrase="Оценка работы менеджера", flag = False):
    """
    Функция выводит часть текста до указанной фразы.
    :param text: Исходный текст
    :param phrase: Фраза, до которой нужно вывести текст
    :return: Строка с обрезанным текстом
    """
    ...
    if index != -1:
        if (flag):
          ...
          return text[index:].strip()  
    return text  # Если фраза не найдена, возвращаем исходный текст



st.title("Анализ текста на выявление запретных фраз и слов паразитов")
tab1, tab2, tab3, tab4 = st.tabs(["Диалог", "Результаты анализа (текст)", "Результаты анализа (таблица)", "Оценка и рекомендации"])
...
with tab2:
    ...
    if uploaded_file is not None:
        ...
        st.markdown(text_result(remove_json(dicts), flag=True))   
...
with tab4:
    if uploaded_file is not None:
        st.markdown(text_result(remove_json(dicts), flag=False))   
